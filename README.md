...
- **Booking.com 集成**：
  - 实时抓取酒店信息
  - 价格、评分、位置等详细数据
  - 🆕 **预订页面URL**：直接跳转到 Booking.com 预订页面
  - 🆕 **酒店图片**：展示酒店真实图片，提升视觉体验
- **智能卡片嵌入**：
  - 🆕 酒店卡片嵌入到推荐文本的对应位置
  - 🆕 每个酒店推荐后自动显示对应的酒店卡片
  - 🆕 支持点击卡片直接跳转预订
...
1. 在聊天界面输入酒店需求，例如："我想订酒店"、"上海市中心，预算500元左右"
2. 系统会实时显示搜索进度（步骤1-4）
3. AI 助手会根据需求推荐合适酒店（最多5个）
4. 🆕 **酒店卡片嵌入**：每个酒店推荐后会自动显示对应的酒店卡片，包含：
   - 酒店图片
   - 酒店名称、价格、评分
   - 位置信息
   - 设施标签
   - 预订按钮（点击直接跳转到 Booking.com）
5. 🆕 **旅行计划关联**：如果有激活的旅行计划，系统会：
   - 自动提取日期信息
   - 根据行程中的景点推荐合适位置的酒店
   - 考虑交通便利性和行程安排
